...
<div id="element" class="hero-body-index">
	<p><font color="white" align="center"><h2>Login</h2></font></p>
	<form method="POST" >
	<table> 
...
<br><br><font color="white"><a href="forgetpassword1.php">forget Password?</font></a>
	</td></tr>
	<tr><td>
	</td><tr>
...
if ($count == 1)
	{
		...
		$user_type=$row['user_type'];
			...
			mysql_query("INSERT INTO history (data,action,date,user)VALUES('$f $l', 'Login', NOW(),'$user_type')")or die(mysql_error());
				...
				header('http://localhost/tour/operator/oper.php');
				session_write_close();
                                        exit();
		}
				...
				$user_type=$row['user_type'];
					...
					mysql_query("INSERT INTO history (data,action,date,user)VALUES('$f $l', 'Login', NOW(),'$user_type')")or die(mysql_error());
					 ...
					 session_start();
						session_regenerate_id();
					$_SESSION['id']=$row['user_id'];
					header('http://localhost/tour/admin/index.php');
					session_write_close();
							exit();
	}
